% normally the code shuffles frame order to get confidence bounds for edge
% potential (node strength), but the algorithm is a lot faster without this step
function [ens_nodes, results] = find_temporal_ens_nodes_noshuf(best_model,data,stimuli, num_controls)
...
%             (epsum>(shuffle_model.mepsum+shuffle_model.sdepsum)));
        core_crf{ii} = find(auc(:,ii)>(mean(auc_ens{ii})+std(auc_ens{ii}))&...
            (epsum>(nanmean(epsum)+nanstd(epsum)/10 ))); % divide by sqrt(num_shuffle), i.e., 10
