...
for ises = 3%1:numel(ses2agg)
clearvars -except ises ses2agg
...
if exist([noshufdir 'crf_noshuf_Rz1pt5_sigBK_' mdcond '.mat'], 'file')
load([noshufdir 'crf_noshuf_Rz1pt5_sigBK_' mdcond '.mat'])
else
...
save([pathpp 'crf_noshuf_Rz1pt5_sigBK.mat'], 'ens_nodes', 'crfresults')
save([noshufdir 'crf_noshuf_Rz1pt5_sigBK_' mdcond '.mat'], 'ens_nodes', 'crfresults')
end
...
annotation('textbox', [0.1 0.91 0.8 0.1], 'string', mousedate, 'interpreter', 'none', 'edgecolor', 'none')
...
for ii = 2:5%1:num_stim
    ...
    for isp =1:4
        ...
        tempneu = ismember(neusigBK, find(neuoi));
        scatter(epsum(tempneu),auc(tempneu,ii),nodesz,'filled','MarkerFaceColor', ttcol(isp,:))
    end
        tempneu = ismember(neusigBK, find(inducerencoder));
    scatter(epsum(tempneu),auc(tempneu,ii),nodesz,'filled','MarkerFaceColor', [0 0 0])
